...
ofile = 'H_test_output.csv'
st1=obspy.read(path1+'/HOOD*E_test*.SAC')
st2=obspy.read(path2+'/HOOD*E_test*.SAC')
  ...
  #         tr1=st1[0]
    ...
    try:
        xcorr=xcorr_pick_correction(
            ...
            plot=False,
            filename=None)
        output.append([evid1,evid2,station,channel,xcorr[1],xcorr[0]])
    except:
        print("No Go: ", evid1, evid2)
        continue
df = pd.DataFrame(output,columns=['evid1','evid2','sta','chn','cc','dt'])
