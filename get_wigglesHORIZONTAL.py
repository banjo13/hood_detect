...
ofile = '/Users/banjo/home/bnjo/hood/util/catalogs/_2002informationHORIZONTAL.csv'
...
#         print('station : ',sta)
        if len(temp) == 0:
            continue
        if len(temp) == 2:
            sinfo = temp[temp['PHA']=='S']
            swave_tt = float(sinfo.ARR.iloc[0])
            origin = sinfo.ABSA.iloc[0]
            origin = utcdt(origin)
            otime = origin - swave_tt
        ...
        if len(temp) == 1:
            ...
            endtime = origin + timelen[1]
        try:
            st = client.get_waveforms('*',sta,'*','HHE,EHE,BHE,HHN,EHN,BHN',starttime,endtime)
        except:
        ...
        for tr in st:
            ...
            names.append(name)
            information.append([name,ID,origin,swave_tt,otime,lat,lon,dep,mag])
            tr.write(name,format='sac')

df= pd.DataFrame(information,columns =["name","ID","origin","swave_tt","otime","lat","lon","dep","mag"])
